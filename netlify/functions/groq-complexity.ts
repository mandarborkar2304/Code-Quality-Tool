...
export const handler: Handler = async (event, context) => {
  ...
  try {
    const { code, language, functionName, context: codeContext } = JSON.parse(event.body || '{}');
...
Respond ONLY with the JSON object, no additional text.`;
    ...
    console.error('Groq complexity analysis error:', error);
    
    // Return fallback analysis
    const fallbackAnalysis = generateFallbackAnalysis(event.body);
 ...
 */
function generateFallbackAnalysis(requestBody: string | null) {
  const { code, language } = JSON.parse(requestBody || '{}');
  const staticAnalysis = performStaticAnalysis(code || '', language || 'javascript');
